...
> 2026-07-22 | 真机分支 (real) | 当前实现版本 v3
...
│                         真机 (Jetson Nano)                            │
│                                                                      │
│  IMX219 CSI → nvarguscamerasrc → TCP :5000 → detector_unified.py     │
│                                                       │              │
│                                                /tmp/vision_pipe      │
│                                                /tmp/recon_pipe       │
│                                                /tmp/h_pipe           │
│                                                       │              │
│  FC TELEM2 ←→ /dev/ttyTHS1:57600 → MAVSDK → droneMission ←┘         │
│       │                                    → /tmp/altitude_pipe      │
│       ├── Servo CH1/CH2                    → /tmp/mission_cmd        │
│       └── GPS / Baro / IMU                                           │
│                                                                      │
│  GCS: 数传 → QGroundControl (监控)                                   │
│  SSH: 笔记本 → WiFi → Jetson (调试)                                   │
└──────────────────────────────────────────────────────────────────────┘
```

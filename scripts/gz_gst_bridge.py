...
挂载点像素投影公式（来自 dxy_apm_ws/CameraGimbal.h::worldToPixel 简化）:
  ...
  pixel_r = world_radius * fx / slantDistance (斜距随高度动态变化)
...
def computeMountPixels(altitudeM):
    """根据当前高度计算两个挂载点的像素坐标和半径 (使用斜距, 随高度动态变化)"""
    if altitudeM < 0.1:
        return None
    ...
    offsetL = math.hypot(d_left_x, d_left_y)
    offsetR = math.hypot(d_right_x, d_right_y)
    slantL = math.sqrt(altitudeM * altitudeM + offsetL * offsetL)
    slantR = math.sqrt(altitudeM * altitudeM + offsetR * offsetR)
    ...
    r_px    = int(WORLD_RADIUS_M * FX / ((slantL + slantR) * 0.5))
...
class GstAppSrcBridge:
    def __init__(self, world_name, tcp_port=None, display=True, altitude=1.2):
        ...
        self.display = display and HAS_GST
    ...
    def buildPipeline(self):
        if not HAS_GST:
            print("[gst] GStreamer Python bindings (gi) not available, skipping pipeline")
            return
        Gst.init(None)
        pipeDesc = (
    ...
    def pushFrame(self, rgbData: bytes):
        if self.appsrc is None or not HAS_GST:
            return
    ...
    def start(self):
        if not HAS_GZ:
            print("[bridge] ERROR: gz.transport not available, cannot connect to Gazebo", flush=True)
            return

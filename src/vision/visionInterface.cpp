...
// ─── reconPipe: text protocol for reconnaissance zone results ──────────
// Receives: "None\n" or "1:color:pct,...;2:empty;...\n"

reconPipe::reconPipe(const std::string& path)
    : path_(path), fd_(-1), running_(false), hasNewData_(false) {}

reconPipe::~reconPipe() { close(); }

bool reconPipe::open() {
    if (mkfifo(path_.c_str(), 0666) != 0 && errno != EEXIST) {
        log("WARNING: recon_pipe mkfifo: " + std::string(strerror(errno)));
    }
    fd_ = ::open(path_.c_str(), O_RDONLY | O_NONBLOCK);
    if (fd_ < 0) {
        log("ERROR: Cannot open recon pipe: " + path_);
        return false;
    }
    running_ = true;
    readerThread_ = std::thread(&reconPipe::readLoop, this);
    log("Recon pipe opened: " + path_);
    return true;
}

void reconPipe::close() {
    running_ = false;
    if (readerThread_.joinable()) readerThread_.join();
    if (fd_ >= 0) { ::close(fd_); fd_ = -1; }
}

void reconPipe::readLoop() {
    char buf[512];
    std::string line;
    fd_set fds;
    timeval tv{0, 50000};
...
bool reconPipe::readLatest(std::string& result) {
    std::lock_guard<std::mutex> lock(mutex_);
    if (!hasNewData_) return false;
    hasNewData_ = false;
...
bool missionCmdPipe::open() {
    ...
    log("Cmd pipe file at: " + path_ + " (will connect when vision reader is ready)");
    return true;
}
